...
TGraphAsymmErrors* getATLASJetRAA_5020_2019(int cent=0)
{
   ...
   if (cent == 0){
      ...
      // 30-40%
      TFile *inf = new TFile ("ATLASJetRAA_5020_2019/HEPData-ins1673184-v1-Table_22.root");
      TGraphAsymmErrors* g = (TGraphAsymmErrors*) inf->Get("Table 22/Graph1D_y1");
      return g;
   }


   // 70-80%
      TFile *inf = new TFile ("ATLASJetRAA_5020_2019/HEPData-ins1673184-v1-Table_26.root");
      TGraphAsymmErrors* g = (TGraphAsymmErrors*) inf->Get("Table 26/Graph1D_y1");
      return g;
...
void plotJetEWCentRAA()
{
   TCanvas *c = new TCanvas("c","",600,600);
   c->SetLogx();
   TGraphAsymmErrors *gATLAS = getATLASJetRAA_5020_2019(0);
   TGraphAsymmErrors *gATLAS_70_80 = getATLASJetRAA_5020_2019(2);
   TGraphAsymmErrors *gATLAS_30_40 = getATLASJetRAA_5020_2019(1);
...
//   gSTAR->Draw("p same");
   ...
   gCMSPhoton->SetMarkerColor(4);
   gCMSPhoton->SetLineColor(4);
   gCMSPhoton->SetMarkerSize(1.5);
   
   
   gATLAS_30_40->SetMarkerStyle(21);
   gATLAS_30_40->SetMarkerSize(1.1);
   
   
   gATLAS_30_40->Draw("p same");
   
   TLegend *leg0 = new TLegend(0.4,0.21,0.9,0.31);
   formatLegend(leg0);
   leg0->AddEntry(gALICE,"PbPb #sqrt{s_{NN}}=5.02 TeV","");
//   leg0->AddEntry(gALICE,"Anti-k_{T} jet R=0.4","");
   leg0->Draw();

   TLegend *leg = new TLegend(0.339,0.681,0.93,0.92);
   formatLegend(leg);
   leg->AddEntry(gALICE,"Anti-k_{T} Jet R=0.4, 0-10%","");
   leg->AddEntry(gALICE,"ALICE, |y|<0.3, PbPb lead track p_{T}> 7 GeV","pl");
   leg->AddEntry(gATLAS,"ATLAS, |y|<2.8","pl");
   leg->AddEntry(gCMS,"CMS, |y|<2.0, 0-10%","pl");
   leg->AddEntry(gALICE,"Anti-k_{T} Jet R=0.4, 30-40%","");
   leg->AddEntry(gATLAS_30_40,"ATLAS, |y|<2.8","pl");
