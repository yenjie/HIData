...
//   TGraphAsymmErrors *gPHENIX = getPHENIXPi0RAA_200_2013();
   ...
   gATLAS_PromptJpsi->SetMarkerStyle(24);
   gATLAS_PromptJpsi->SetMarkerSize(1.5);
   gATLAS_PromptJpsi->SetMarkerColor(4);
...
//   leg0->Draw();
   ...
   TLegend *leg = new TLegend(0.274,0.711,0.918,0.913);
   formatLegend(leg);
   leg->AddEntry(gALICE_InclusiveJpsi,"PbPb #sqrt{s_{NN}}=5.02 TeV","");
   leg->AddEntry(gALICE_InclusiveJpsi,"ALICE Inclusive J/#psi, 0-20%, |y|<0.9","pl");
   leg->AddEntry(gALICE_InclusiveJpsiForward,"ALICE Inclusive J/#psi, 0-20%, 2.5<y<4.0","pl");
   leg->AddEntry(gATLAS_PromptJpsi,"ATLAS Prompt J/#psi, 0-10%, |y|<2","pl");
   leg->AddEntry(gCMS_PromptJpsi,"CMS Prompt J/#psi, 0-10%, |y|<2.4","pl");
//   leg->AddEntry(gSTAR,"STAR h^{#pm}, AuAu 0.2 TeV 0-5%, |y|<0.5","pl");
//   leg->AddEntry(gPHENIX,"PHENIX #pi^{0}, AuAu 0.2 TeV 0-10%, |y|<0.5","pl");
   leg->Draw();
   
   TLine *l = new TLine(0.5,1,40,1);
   l->SetLineStyle(2);
   l->Draw();
