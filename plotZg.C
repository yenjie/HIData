...
void plotZg()
{
   ...
   TLegend *leg0 = new TLegend(0.09699,0.25,0.59699,0.35);
   formatLegend(leg0);
//   leg0->AddEntry(gALICE,"PbPb #sqrt{s_{NN}}=5.02 TeV, 0-10%","");
   leg0->AddEntry(gALICE_Zg,"0-10% Pb+Pb / pp reference","");
   leg0->AddEntry(gALICE_Zg,"Anti-k_{T} R=0.4, #Delta R>0.1","");
   leg0->Draw();

   TLegend *leg = new TLegend(0.2,0.72,0.9,0.92);
   formatLegend(leg);
   leg->AddEntry(gALICE_Zg,"ALICE 80<p_{T}<120 GeV, PbPb 2.76 TeV/PYTHIA","pl");
//   leg->AddEntry(gATLAS,"ATLAS, |y|<2.8","pl");
   leg->AddEntry(gCMS_Zg,"CMS 160<p_{T}<180 GeV, PbPb 5.02 TeV/pp","pl");
   leg->Draw();
